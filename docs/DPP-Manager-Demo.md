...
The companion demo, [`dpp-service/docs/DPP-Service-Demo.md`](../../dpp-service/docs/DPP-Service-Demo.md),
asks *who owns the product passport?* and answers it on the command line, with
`curl`, a signing script and a shell that knows what a DID is.

This one asks what is left of that answer when the economic operator is a lamp
manufacturer with no interest in any of it. The claim under test:

> Holding your own identifier, your own keys and your own data must not require
> a person who can operate a terminal. One container, one file, one passphrase.
...
| | |
|---|---|
| DPP Manager | a container on the operator's own machine, `http://localhost:3000` |
| DPP Service | `https://dpp-service.ownyourdata.eu` |
| VDR / registrar for `did:oyd` | `https://oydid.ownyourdata.eu` |
| SOyA repository | `https://soya.ownyourdata.eu`, and a copy inside the image |

**Where things are.** Three places, and the whole demo is about which of them
holds what:

```
┌─ THIS MACHINE ──────────────────────────── one encrypted file ─┐
...
```bash
docker run -d --name dpp-manager -p 3000:3000 -v dpp-manager-data:/data dpp-manager
open http://localhost:3000
```

That is the whole installation. The image carries Rails, SQLCipher, the SOyA
toolchain and the form renderer; the volume carries one file. Nothing is
configured yet, and nothing needs to be.
...
Creating the file also fills it: the product types the image ships with are
installed at that moment, which is why the button takes a second. From here on
the application works with the network unplugged, up to the point where
something has to leave the machine.
...
Step two mints the operator's identity. It happens **on this machine**: the
...
## 3 — What the image already knows
...
element model of EN 18223. The LED lamp comes with the image — structure, form,
transformation and the transformation for reading one back — so a fresh
installation with no network still has a working type. "Fetch again" asks the
repository and falls back to the copy inside the image, in that order.
...
This is the step that leaves the machine, and the warning is exact about what
...
> can never revoke that identifier. Only this machine can.
...
One file. Changing the passphrase re-encrypts it; a backup is a copy of it. No
`-wal`, no `-shm`, nothing beside it — which is why "make a backup" is one
button and not a procedure.
...
**What the image ships with:** Rails 8 on Ruby 3.3, SQLCipher built from source,
soya-web-cli and soya-form from the SOyA repository, and the LED lamp structure
with both transformations. One `docker run`, one volume.

**What is implemented but not shown here:** custody at a data intermediary — the
mandate the operator signs (`docs/Delegation.md` §5), its renewal, the
comparison with what the service holds, and handing a passport to a different
custodian, identifier and all. All of it is in the application and under test;
the live demo needs a collection at a pod whose controller is this
installation's identity DID.
